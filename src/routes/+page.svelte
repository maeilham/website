<script lang="ts">
	import { page } from "$app/stores";
	import HomeTerminal from "$lib/components/HomeTerminal.svelte";

	// ── URL 상태 ──────────────────────────────────────────────────────────────
	const urlAction = $derived($page.url.searchParams.get("action"));
	const urlToken  = $derived($page.url.searchParams.get("token"));
	const showUnsubscribeModal = $derived(urlAction === "unsubscribe" && !!urlToken);

</script>

<svelte:head>
	<title>매일함</title>
</svelte:head>

<div class="page">

	<header class="header">
		<span class="logo">매일함</span>
	</header>

	<main class="content">

		<div class="intro">
			{#if showUnsubscribeModal}
				<p class="unsub-label">구독 해지</p>
				<h1 class="intro-title">구독을 취소할게요.</h1>
				<p class="intro-desc">터미널에서 확인해주세요.</p>
			{:else}
				<h1 class="intro-title">매일 아침,<br>질문 하나가 도착합니다.</h1>
				<p class="intro-desc">
					아는 것 같았는데 막히는 것들 — 백엔드, 프론트엔드, CS 기초를
					매일 한 통씩 받아보세요. GitHub Discussion에서 함께 답을 만들어가요.
				</p>
			{/if}
		</div>

		<div class="terminal-wrap">
			<HomeTerminal action={urlAction ?? ''} token={urlToken ?? ''} />
		</div>

	</main>

	<footer class="footer">
		<span>© 2026 maeilham</span>
		<a href="https://github.com/maeilham" target="_blank" rel="noreferrer" class="github-link" aria-label="GitHub">
			<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
				<path d="M12 2C6.477 2 2 6.477 2 12c0 4.418 2.865 8.166 6.839 9.489.5.092.682-.217.682-.482 0-.237-.009-.868-.013-1.703-2.782.604-3.369-1.342-3.369-1.342-.454-1.155-1.11-1.462-1.11-1.462-.908-.62.069-.608.069-.608 1.003.07 1.531 1.03 1.531 1.03.892 1.529 2.341 1.087 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.11-4.555-4.943 0-1.091.39-1.984 1.029-2.683-.103-.253-.446-1.27.098-2.647 0 0 .84-.269 2.75 1.025A9.578 9.578 0 0 1 12 6.836a9.59 9.59 0 0 1 2.504.337c1.909-1.294 2.747-1.025 2.747-1.025.546 1.377.202 2.394.1 2.647.64.699 1.028 1.592 1.028 2.683 0 3.842-2.339 4.687-4.566 4.935.359.309.678.919.678 1.852 0 1.336-.012 2.415-.012 2.743 0 .267.18.578.688.48C19.138 20.163 22 16.418 22 12c0-5.523-4.477-10-10-10z"/>
			</svg>
		</a>
	</footer>

</div>

<style>
	:global(body) {
		background: #fff;
		font-family: 'SF Pro KR', 'SF Pro Display', -apple-system, BlinkMacSystemFont, 'Apple SD Gothic Neo', 'Noto Sans KR', sans-serif;
		color: #333d4b;
	}

	.page {
		min-height: 100vh;
		display: flex;
		flex-direction: column;
		max-width: 600px;
		margin: 0 auto;
		padding: 0 24px;
	}

	.header {
		padding: 48px 0 64px;
	}
	.logo {
		font-size: 16px;
		font-weight: 700;
		color: #333d4b;
		letter-spacing: -0.2px;
	}

	.content {
		flex: 1;
		display: flex;
		flex-direction: column;
		gap: 48px;
	}

	.intro {
		display: flex;
		flex-direction: column;
		gap: 16px;
	}
	.intro-title {
		font-size: 28px;
		font-weight: 700;
		line-height: 1.4;
		color: #333d4b;
		letter-spacing: -0.5px;
		margin: 0;
		word-break: keep-all;
	}
	.intro-desc {
		font-size: 16px;
		color: #8b95a1;
		line-height: 1.7;
		margin: 0;
		word-break: keep-all;
	}

	.terminal-wrap {
		width: 100%;
	}

	/* ── 해지 ── */
	.unsub-label {
		font-size: 12px;
		font-weight: 600;
		color: #8b95a1;
		letter-spacing: 0.4px;
		text-transform: uppercase;
		margin: 0;
	}

	.footer {
		padding: 48px 0 36px;
		font-size: 13px;
		color: #8b95a1;
		display: flex;
		align-items: center;
		justify-content: space-between;
	}
	.github-link {
		color: #8b95a1;
		display: flex;
		transition: color 0.15s;
	}
	.github-link:hover { color: #333d4b; }

	@media (max-width: 480px) {
		.intro-title { font-size: 28px; }
	}
</style>
